...
\author{
  Yuchi Dai\thanks{\texttt{scyyd12@nottingham.edu.cn}} \and
  Yibei Jiang\thanks{\texttt{smyyj8@nottingham.edu.cn}} \and
  Xinyu Chen\thanks{\texttt{sayxc6@nottingham.edu.cn}} \and
  Tongda Deng\thanks{\texttt{smytd1@nottingham.edu.cn}}\\[0.5em]
  \normalsize University of Nottingham Ningbo China\\
  \normalsize Nottingham Mathematical Modelling Competition 2026
}
\date{17 May 2026}
...
=
\begin{pmatrix}
 \mathrm{diag}(\gamma_i + \mu_i + \sigma_i) - \mathbf{T}^{\top} & \mathbf{0} \\
...
\textbf{Key implication:} Complete eradication of endemic AMR requires both $R_0 < 1$ (satisfied under baseline) \emph{and} near-elimination of admission colonisation ($\alpha_i \to 0$). Scenario B moves strongly in this direction by reducing $\alpha_i$ by 80\%, but does not set it exactly to zero. Reducing $R_0$ further through hand hygiene or antibiotic stewardship is insufficient on its own.
...
We recommend \textbf{Scenario B (universal admission screening + contact precautions)} as the primary intervention for reducing endemic AMR prevalence. \textbf{Scenario C (antibiotic stewardship)} should be considered as a secondary follow-on policy for long-term sustainability, but the combined B+C package was not simulated as a standalone scenario in the present study.

The evidence basis for this recommendation is threefold:
\begin{enumerate}
  \item \textbf{Efficacy:} Scenario B achieves the largest absolute prevalence reduction (6.3 percentage points system-wide), driven by strong attenuation of the admission colonisation forcing term.
  \item \textbf{Mechanism:} Sensitivity analysis identifies transmission rate $\beta$ (PRCC = 0.95) and admission colonisation rate $\alpha$ (PRCC = 0.92) as the two strongest drivers of 12-month prevalence; Scenario B directly targets both.
  \item \textbf{Literature support:} Lipsitch et al. \cite{lipsitch2000} emphasise that controlling admission colonisation alongside in-hospital transmission is central to reducing endemic resistance; Scenario B targets both pathways simultaneously.
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{4.1cm}cc}
    ...
    \midrule
    $\beta$ (transmission) & \textbf{+0.993} & +0.950 \\
    $\gamma$ (decolonisation) & \textbf{$-0.956$} & $-0.876$ \\
    $\delta$ (hand hygiene) & $-0.515$ & $-0.136$ \\
    $\alpha$ (admission rate) & $-0.019 \approx 0$ & \textbf{+0.917} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{$R_0$ sensitivity:} $\beta$ ($+0.993$) and $\gamma$ ($-0.956$) dominate $R_0$ sensitivity, reflecting their direct roles in new colonisations and recovery. The hand hygiene parameter $\delta$ has moderate PRCC ($-0.515$), indicating that the HCW-mediated route contributes to $R_0$ but is not the dominant pathway under the present parameterisation. The admission colonisation rate $\alpha$ has negligible PRCC with $R_0$ ($-0.019$), as expected: $R_0$ is computed at the disease-free equilibrium where $\alpha$ is formally set to zero, so $\alpha$ does not enter the NGM calculation.

\textbf{Prevalence sensitivity:} The ranking changes substantially for 12-month prevalence. The dominant factors are $\beta$ (+0.950) and $\alpha$ (+0.917), while $\delta$ drops to $-0.136$. This reveals a critical asymmetry: hand hygiene compliance ($\delta$) has visible leverage on $R_0$ but limited independent effect on steady-state prevalence, whereas imported colonisation ($\alpha$) has almost no effect on $R_0$ but exerts near-maximal influence on the endemic outcome. Scenario B is therefore powerful precisely because it targets both high-impact prevalence drivers at once.
...
\begin{enumerate}
  ...
  \item \textbf{Transmission and imported colonisation jointly dominate endemic prevalence:} Sensitivity analysis reveals that the transmission rate $\beta$ (PRCC 0.95) and admission colonisation rate $\alpha$ (PRCC 0.92) are the two strongest drivers of 12-month prevalence, while $\alpha$ remains negligible in $R_0$. This asymmetry means that $R_0$ alone is an incomplete guide to intervention priority.
